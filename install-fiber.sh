...
uninstall_node() {
  ...
  if check_command ufw; then
    ...
    sudo ufw delete deny 3001/tcp 2>/dev/null || true
...
setup_firewall() {
  ...
  if confirm "Set up firewall rules now? (This will enable ufw if not already running)"; then
    ...
    ok "Dashboard (port 3001): blocked from internet (use SSH tunnel to access)"
...
print_summary() {
  ...
  echo "    ${INSTALL_DIR}/start-dashboard.sh       - start the dashboard manually"
...
STARTEOF
  ...
  # Generate start-dashboard.sh - for manual dashboard launch without systemd
  DASHBOARD_APP_REL="dashboard/fiber-dashboard"
  cat > "$INSTALL_DIR/start-dashboard.sh" << DASHEOF
#!/usr/bin/env bash
# Fiber Dashboard manual starter - generated by install-fiber.sh
INSTALL_DIR="$INSTALL_DIR"
DASHBOARD_APP="\$INSTALL_DIR/$DASHBOARD_APP_REL"
export FIBER_RPC_URL="http://localhost:8227"
export PORT="3001"
export BIND_HOST="127.0.0.1"
export NODE_ENV="production"
export CKB_CLI_PATH="/usr/local/bin/ckb-cli"
cd "\$DASHBOARD_APP"
exec "\$DASHBOARD_APP/node_modules/.bin/tsx" server/index.ts
DASHEOF
  chmod +x "$INSTALL_DIR/start-dashboard.sh"
  ok "start-dashboard.sh written to $INSTALL_DIR/start-dashboard.sh"
