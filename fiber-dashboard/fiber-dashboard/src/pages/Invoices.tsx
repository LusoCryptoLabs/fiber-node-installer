  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card space-y-4">
        ...
        {createMut.isError && (
          <div className="alert-red rounded-md p-3 text-sm">
        ...
        {generatedInvoice && (
          <div className="space-y-3 pt-2 border-t border-border">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-text-primary">Invoice Generated</h3>
              {paid ? (
            ...
            <div className="bg-bg-surface rounded-md p-3 font-mono text-xs break-all text-text-primary leading-relaxed">
            ...
            <div className="text-xs text-text-muted space-y-1">
              {amountAttr && (
                <div>Amount: <span className="text-text-primary">{shannonsToCkb(BigInt(amountAttr || "0x0").toString())} CKB</span></div>
              )}
              {paymentHash && (
                <div>Payment hash: <span className="mono text-text-secondary">{paymentHash.slice(0, 20)}…</span></div>
              )}
            </div>
          </div>
        )}
              ...
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {inv.description && (
                      <span className="text-xs text-text-secondary italic">"{inv.description}"</span>
                    )}
                  </div>
                  <div className="mono text-xs text-text-muted truncate">{inv.payment_hash}</div>
        ...
        {parsedInvoice && (
          <div className="bg-bg-surface rounded-md p-4 space-y-2 text-sm">
            <div className="flex items-center justify-between mb-3">
              <span className="font-semibold text-text-primary">Invoice Details</span>
              {statusBadge(parsedInvoice.status)}
            </div>
            <div className="grid grid-cols-2 gap-y-2 text-xs">
              <span className="text-text-muted">Currency</span>
              <span className="text-text-primary">{parsedInvoice.invoice.currency}</span>
              ...
              <span className="text-text-muted">Payment Hash</span>
              <span className="mono text-text-secondary break-all">
              ...
              {parsedInvoice.invoice.data.attrs.map((attr, i) =>
                attr.type === "Description" ? (
                  <span key={i} className="contents">
                    ...
                    <span className="text-text-primary">{String(attr.value)}</span>
                  </span>
                ) : null
              )}
            </div>
